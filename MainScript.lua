...
local GuiLibrary
local baseDirectory = (shared.VapePrivate and "HT/" or "HT/")
...
assert(not shared.VapeExecuted, "HT Already Injected")
shared.VapeExecuted = true

for i,v in pairs({baseDirectory:gsub("/", ""), "HT", baseDirectory.."Libraries", baseDirectory.."CustomModules", baseDirectory.."Profiles", "HT/assets"}) do 
	if not isfolder(v) then makefolder(v) end
end
task.spawn(function()
	local success, assetver = pcall(function() return vapeGithubRequest("assetsversion.txt") end)
	if not isfile("HT/assetsversion.txt") then writefile("HT/assetsversion.txt", "0") end
	if success and assetver > readfile("HT/assetsversion.txt") then
		redownloadedAssets = true
		if isfolder("HT/assets") and not shared.VapeDeveloper then
			if delfolder then
				delfolder("HT/assets")
				makefolder("HT/assets")
			end
		end
		writefile("HT/assetsversion.txt", assetver)
	end
end)
...
task.spawn(function()
	...
	image.Image = downloadVapeAsset("HT/assets/CombatIcon.png")
	...
	task.spawn(function()
		task.wait(15)
		if image and image.ContentImageSize == Vector2.zero and (not errorPopupShown) and (not redownloadedAssets) and (not isfile("HT/assets/check3.txt")) then 
            errorPopupShown = true
            displayErrorPopup("Assets failed to load, Try another executor (executor : "..(identifyexecutor and identifyexecutor() or "Unknown")..")", function()
                writefile("HT/assets/check3.txt", "")
            end)
        end
	end)
end)
...
local Combat = GuiLibrary.CreateWindow({
	Name = "Combat", 
	Icon = "HT/assets/CombatIcon.png", 
	IconSize = 15
})
local Blatant = GuiLibrary.CreateWindow({
	Name = "Blatant", 
	Icon = "HT/assets/BlatantIcon.png", 
	IconSize = 16
})
local Render = GuiLibrary.CreateWindow({
	Name = "Render", 
	Icon = "HT/assets/RenderIcon.png", 
	IconSize = 17
})
local Utility = GuiLibrary.CreateWindow({
	Name = "Utility", 
	Icon = "HT/assets/UtilityIcon.png", 
	IconSize = 17
})
local World = GuiLibrary.CreateWindow({
	Name = "World", 
	Icon = "HT/assets/WorldIcon.png", 
	IconSize = 16
})
local Friends = GuiLibrary.CreateWindow2({
	Name = "Friends", 
	Icon = "HT/assets/FriendsIcon.png", 
	IconSize = 17
})
...
local Profiles = GuiLibrary.CreateWindow2({
	Name = "Profiles", 
	Icon = "HT/assets/ProfilesIcon.png", 
	IconSize = 19
})
GUI.CreateDivider()
GUI.CreateButton({
...
ProfilesTextList = Profiles.CreateTextList({
	...
	RemoveFunction = function(profileIndex, profileName) 
		if profileName ~= "default" and profileName ~= GuiLibrary.CurrentProfile then 
			pcall(function() delfile(baseDirectory.."Profiles/"..profileName..(shared.CustomSaveVape or game.PlaceId)..".HTprofile.txt") end)
...
OnlineProfilesButtonImage.Image = downloadVapeAsset("HT/assets/OnlineProfilesButton.png")
...
OnlineProfilesExitButton.Image = downloadVapeAsset("HT/assets/ExitIcon1.png")
...
OnlineProfilesFrameShadow.Image = downloadVapeAsset("HT/assets/WindowBlur.png")
...
OnlineProfilesFrameIcon.Image = downloadVapeAsset("HT/assets/ProfilesIcon.png")
...
OnlineProfilesButton.MouseButton1Click:Connect(function()
	...
	if not profilesLoaded then
        ...
        local success, result = pcall(function()
            return game:GetService("HttpService"):JSONDecode(game:HttpGet("https://raw.githubusercontent.com/HTIsOnTop/HTProfiles/main/Profiles/"..saveplaceid.."/profilelist.txt", true))
        end)
		for i,v in pairs(success and result or {}) do 
			onlineprofiles[i] = v
		end
		for i2,v2 in pairs(onlineprofiles) do
			local profileurl = "https://raw.githubusercontent.com/HTIsOnTop/HTProfiles/main/Profiles/"..saveplaceid.."/"..v2.OnlineProfileName
			...
			profiledownload.MouseButton1Click:Connect(function()
				writefile(customdir.."Profiles/"..v2.ProfileName..saveplaceid..".HT.txt", game:HttpGet(profileurl, true))
...
local TextGUI = GuiLibrary.CreateCustomWindow({
	Name = "Text GUI", 
	Icon = "HT/assets/TextGUIIcon1.png", 
	IconSize = 21
})
local TextGUICircleObject = {CircleList = {}}
GUI.CreateCustomToggle({
	Name = "Text GUI", 
	Icon = "HT/assets/TextGUIIcon3.png",
...
VapeLogo.Image = downloadVapeAsset("HT/assets/HTitle.png")
...
VapeLogoV4.Image = downloadVapeAsset("HT/assets/HTVersionV2.png")
...
TextGUIMode = TextGUI.CreateDropdown({
	...
	Function = function(val)
		...
		if val == "Drawing" then
			...
			VapeLogoDrawing.Data = readfile("HT/assets/HTitle.png")
			...
			VapeLogoV4Drawing.Data = readfile("HT/assets/HTVersionV2.png")
			...
			VapeLogoShadowDrawing.Data = readfile("HT/assets/HTitle.png")
			...
			VapeLogo4Drawing.Data = readfile("HT/assets/HTVersionV2.png")
...
TextGUI.CreateToggle({
	...
	HoverText = "Renders a HT watermark"
})
...
local TargetInfo = GuiLibrary.CreateCustomWindow({
	Name = "Target Info",
	Icon = "HT/assets/TargetInfoIcon1.png",
	IconSize = 16
})
...
TargetInfoHealthBackgroundShadow.Image = downloadVapeAsset("HT/assets/WindowBlur.png")
...
GUI.CreateCustomToggle({
	Name = "Target Info", 
	Icon = "HT/assets/TargetInfoIcon2.png", 
    ...
    if (not teleportedServers) and (not shared.VapeIndependent) then
		...
		local teleportScript = [[
			...
			if shared.VapeDeveloper then 
				loadstring(readfile("HT/NewMainScript.lua"))() 
			else 
				loadstring(game:HttpGet("https://raw.githubusercontent.com/HTIsOnTop/HT/5a8e3a9e9ffca08983dcf948c8d00c781c34a4d8/NewMainScript.lua", true))() 
			end
		]]
		if shared.VapeDeveloper then
			teleportScript = 'shared.HTDeveloper = true\n'..teleportScript
		end
		if shared.VapePrivate then
			teleportScript = 'shared.HTPrivate = true\n'..teleportScript
		end
		if shared.VapeCustomProfile then 
			teleportScript = "shared.HTCustomProfile = '"..shared.VapeCustomProfile.."'\n"..teleportScript
		end
		GuiLibrary.SaveSettings()
...
local function loadVape()
	if not shared.VapeIndependent then
		...
		if isfile("HT/CustomModules/"..game.PlaceId..".lua") then
			loadstring(readfile("HT/CustomModules/"..game.PlaceId..".lua"))()
		else
			local suc, publicrepo = pcall(function() return game:HttpGet("https://raw.githubusercontent.com/HTIsOnTop/HT/"..readfile("HT/commithash.txt").."/CustomModules/"..game.PlaceId..".lua") end)
			if suc and publicrepo and publicrepo ~= "404: Not Found" then
				writefile("HT/CustomModules/"..game.PlaceId..".lua", "--This watermark is used to delete the file if its cached, remove it to make the file persist after commits.\n"..publicrepo)
				loadstring(readfile("HT/CustomModules/"..game.PlaceId..".lua"))()
			end
		end
		if shared.VapePrivate then
			if isfile("HTprivate/CustomModules/"..game.PlaceId..".lua") then
				loadstring(readfile("HTprivate/CustomModules/"..game.PlaceId..".lua"))()
			end	
		end
	else
